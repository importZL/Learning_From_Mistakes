...
parser.add_argument('--note', type=str, default='cifar100-PAM', help='experiment name')
...
def main():
    ...
    # create model
    model = build_model(len_train)  # W1
    meta_model = build_model(len_train)  # W2
    ...
    scheduler_meta = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer_meta, float(args.epochs), eta_min=1e-4)
...
def train(train_loader,
    ...
    if epoch < args.epoch * 3/10:
        lr = args.lr
    elif epoch < args.epoch * 6/10:
        lr = args.lr * 0.2
    elif epoch < args.epoch * 8/10:
        lr = args.lr * 0.2 ** 2
    else:
        lr = args.lr * 0.2 ** 3
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # step 1
        y_f = model(input_var)
        cost_w = F.cross_entropy(y_f, target_var.long(), reduction='none')
        # cost_v = torch.reshape(cost_w, (len(cost_w), 1))
        prec_train = accuracy(y_f.data, target_var.data, topk=(1,))[0]
        l_f = (cost_w * model.weight_b[:, i]).mean()
        ...
        cost = F.cross_entropy(y_f_hat, target_var.long(), reduction='none')
        cost_v = torch.reshape(cost, (len(cost), 1))
        v_lambda = _compute_a(model, encoder, r_vec, input_var, target_var, input_validation_var,
                              target_validation_var)
        l_f_meta = (cost_v * v_lambda).mean()
        optimizer_meta.zero_grad()
        l_f_meta.backward()
        nn.utils.clip_grad_norm_(meta_model.params(), 5)
        optimizer_meta.step()
        ...
        model_old = _compute_unrolled_model(model, input_var, target_var, lr, optimizer_a)
        meta_model_old = _compute_unrolled_model(meta_model, input_validation_var, target_validation_var, lr, optimizer_meta)

        y_f_hat = meta_model_old(input_var)
        cost = F.cross_entropy(y_f_hat, target_var.long(), reduction='none')
        cost_v = torch.reshape(cost, (len(cost), 1))
        v_lambda = _compute_a(model, encoder, r_vec, input_var, target_var, input_validation_var,
                              target_validation_var)
        l_f_meta = torch.sum(cost_v * v_lambda).mean()
        grads = torch.autograd.grad(l_f_meta, (meta_model_old.params()), create_graph=True)

        y_g_hat = meta_model(input_validation_var)
        l_g_meta = F.cross_entropy(y_g_hat, target_validation_var.long())
        # l_g_meta.backward(retain_graph=True)
        prec_meta = accuracy(y_g_hat.data, target_validation_var.data, topk=(1,))[0]

        grad_l_meta = torch.autograd.grad(l_g_meta, (meta_model.params()))
        ...
        d_lvalw2p_alpha = [v.grad for v in model.b_parameters()]  # should be zero

        d_ailtr_w2_dot_d_lval_w2p__w1p = [v.grad.data for v in model.params()]
        finite_diff_1 = _hessian_vector_product1(model_old, d_ailtr_w2_dot_d_lval_w2p__w1p, input_var, target_var, i)
        ...
        optimizer_r.step()
        optimizer_b.step()
        # print(model.weight_b.reshape(1, -1))
...
def _construct_model_from_theta(model, theta):
        model_new = build_model(len_train)
        model_dict = model.state_dict()

        params, offset = {}, 0
        for k, v in model.named_params_zl():
...
def _compute_unrolled_model(model, input, target, eta, network_optimizer):
        logits = model(input)
        loss = criterion(logits, target)  # 首先计算一下前向损失
        theta = _concat(model.params()).data  # 将所有权重copy一下
        try:  # 替代了optimizer.step() 从而不改变网络权重
            moment = _concat(network_optimizer.state[v]['momentum_buffer']
                             for v in model.params()).mul_(args.momentum)
        except:
            moment = torch.zeros_like(theta)
        dtheta = _concat(torch.autograd.grad(
            loss, model.params())).data + args.weight_decay * theta
        unrolled_model = _construct_model_from_theta(model,
...
def build_model(len_train):
    model = ResNet32(args.dataset == 'cifar10' and 10 or 100, len_train, args.batch_size)
