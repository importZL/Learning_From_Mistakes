...
parser.add_argument('--save', type=str, default='./log-ResNet32', help='save path')
parser.add_argument('--note', type=str, default='cifar100-LFM-flip2', help='experiment name')
...
def adjust_learning_rate(optimizer, epochs):
    ...
    # lr = args.lr * ((0.1 ** int(epochs >= 60)) * (0.1 ** int(epochs >= 80)) * (0.1 ** int(epochs >= 100)))
...
def train(train_loader,
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        # lr = args.lr * ((0.1 ** int(epoch >= 60)) * (0.1 ** int(epoch >= 80)) * (0.1 ** int(epoch >= 100)))
        inputs, targets = inputs.to(device), targets.to(device)
        try:
...
scheduler_b = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer_b, float(args.epochs), eta_min=1e-3)
